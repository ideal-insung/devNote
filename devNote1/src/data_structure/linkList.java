...
public class linkList {
	...
	public MyNode searchNode(int idx) { //인덱스로 찾고하자는 노드리턴
		MyNode node = head; 
	...
	public void addLast(Object data) {
		...
		if(n == null) addFirst(data); //노드가 없다면 firstadd함수
			...
			MyNode addNode = new MyNode(data); //새로운 노드 인스턴스 생성
	...
	public boolean add(int idx, Object data) {
		if(idx < 0 ) throw new RuntimeException();
			...
			MyNode addNode = new MyNode(data); //노드 인스턴스 생성
			MyNode tmp = searchNode(idx);    //서치할노드 템프노드에 저장
	...
	public void remove(int idx) {
		if(idx < 1 || size < idx) throw new RuntimeException();
			...
			MyNode tmpNode = searchNode(idx - 1); //템프노드에 저장
